import {
  initializePicturesGallery,
  updatePicturesGallery,
} from "../components/PicturesGallery.js";
import {
  initializePicturesList,
  updatePicturesList,
} from "../components/PicturesList.js";
import {
  initializePicturesCarousel,
...
import {
  initializeFavorites,
  updatePicturesFavorites,
} from "./FavoritePage.js";
...
const sortPictures = (asc = true) => {
  if (asc) {
    //sort from a to z
    displayPicturesArr.sort((a, b) =>
      a.additionalDetails.title.localeCompare(b.additionalDetails.title)
    );
  } else {
    //sort from z to a
    displayPicturesArr.sort((a, b) =>
      b.additionalDetails.title.localeCompare(a.additionalDetails.title)
    );
  }
  updateDisplays();
};
    ...
    for (let userIdx = 0; userIdx < users.length; userIdx++) {
      ...
      if (users[userIdx].id === token.id) {
        localStorage.setItem(
          ...
          JSON.stringify({
            id: users[userIdx].id,
            first_name: users[userIdx].first_name,
            last_name: users[userIdx].last_name,
            email: users[userIdx].email,
            isAdmin: users[userIdx].isAdmin,
...
const showPopup = (id) => {
  let selectedPicture = displayPicturesArr.find(
...
const addNewFav = (id) => {
  ...
  if (users && userToken) {
    ...
    for (
      let currFavoriteIndex = 0;
      currFavoriteIndex < currUser.favorites.length;
      currFavoriteIndex++
    ) {
      if (currUser.favorites[currFavoriteIndex] == id) {
        showToast(
          "Cannot Favorite this",
          "Picture is already favorited",
          false
        );
        return;
      }
